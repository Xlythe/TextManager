...
public class RetryScheduler implements Observer {
    ...
    public void update(Observable observable) {
        try {
            ...
            Log.v("Retry Scheduler", "[RetryScheduler] update " + observable);
    ...
    private void scheduleRetry(Uri uri) {
        ...
        Cursor cursor = mContentResolver.query(uriBuilder.build(), null, null, null, null);
            ...
            try {
                if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                    ...
                    if ((retryIndex < scheme.getRetryLimit()) && retry) {
                        ...
                        Log.v("Retry Scheduler", "scheduleRetry: retry for " + uri + " is scheduled at "
                        ...
                        if (isRetryDownloading) {
                            Cursor c = mContext.getContentResolver().query(uri, new String[]{Mms.THREAD_ID}, null, null, null);
                            ...
                            mContext.getContentResolver().update(uri, readValues, null, null);
                    ...
                    mContentResolver.update(PendingMessages.CONTENT_URI, values, PendingMessages._ID + "=" + id, null);
                }
            } finally {
                cursor.close();
            }
    ...
    private int getResponseStatus(long msgID) {
        int respStatus = 0;
        Cursor cursor = mContentResolver.query(Mms.Outbox.CONTENT_URI, null, Mms._ID + "=" + msgID, null, null);
        try {
        ...
        if (respStatus != 0) {
            Log.e("RetryScheduler", "Response status is: " + respStatus);
        }
        return respStatus;
    }
    ...
    private int getRetrieveStatus(long msgID) {
        ...
        Cursor cursor = mContentResolver.query(Mms.Inbox.CONTENT_URI, null, Mms._ID + "=" + msgID, null, null);
        try {
            if (cursor.moveToFirst()) {
        ...
        if (retrieveStatus != 0) {
            Log.v("Retry Scheduler", "Retrieve status is: " + retrieveStatus);
        }
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    ...
                    Log.v("Retry Scheduler", "Next retry is scheduled at" + (retryAt - System.currentTimeMillis()) + "ms from now");
                }
            } finally {
                cursor.close();
            }
